...
if __name__ == '__main__':
    ...
    tiltcorrlabels = ['none', 'DR', 'TR']
    timelaglabels = ['none', '', 'covmax-def', 'covmax']
    turbfluctlabels = ['block', 'detrend', 'running', 'exp-running']
    pbar = tqdm(
        itertools.product(
            [0, 1, 2],
            [0, 2, 3],
    ...
    for tiltcorr, timelag, turbfluct in pbar:
        pbar.set_postfix(dict(Tilt=tiltcorrlabels[tiltcorr], Timelag=timelaglabels[timelag]), Fluctuations=turbfluctlabels[turbfluct])
