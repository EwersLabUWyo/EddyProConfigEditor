...
if __name__ == '__main__':
    ...
    if args.pf_file:
        ...
        methods = ['none', 'covariance_maximization_with_default', 'covariance_maximization']
        for tl_method in methods:
            pf_new.Adv.Proc.set_timelag_compensations(method=tl_method)
            pf_new.Basic.set_output_id(output_id=tl_method.replace('_', '-'))
            pf_new.to_eddypro_parallel(
                ini_dir=ini_dir / tl_method, 
                out_path=out_path,
                num_workers=n_cores//len(methods),
                file_duration=30,
                min_worker_timespan=1)
