...
class eddypro_ConfigParser(configparser.ConfigParser):
    ...
    class _Basic:
        ...
        def set_NorthReference(
            ...
            self.root.set('RawProcess_General', 'use_geo_north', str(int(method == 'geo')))
            if method == 'geo':
                ...
                self.root.set('RawProcess_General', 'mag_dec', str(magnetic_declination))
        ...
        def get_NorthReference(self) -> dict:
            ...
            dec_date = datetime.datetime.strptime(self.root.get('RawProcess_General', 'dec_date'), r'%Y-%m-%d')
    ...
    class _Advanced:
        ...
        class _Processing:    
            ...
            def get_AxisRotationsForTiltCorrection(self) -> dict:
                ...
                if method in ['planar_fit', 'planar_fit_nvb']:
                    ...
                    while True:
                        try:
                            exclude = int(self.root.get('RawProcess_TiltCorrection_Settings', f'pf_sector_{n}_exclude'))
                            width = float(self.root.get('RawProcess_TiltCorrection_Settings', f'pf_sector_{n}_width'))
            ...
            def set_TimelagCompensations(
                ...
                if method == 4:
                    ...
                    if autoopt_file is not None:
                        self.root.set('RawProcess_TimelagOptimization_Settings', 'to_file', str(autoopt_file))
                        self.root.set('RawProcess_TimelagOptimization_Settings', 'to_mode', str(0))
            ...
            def get_TimelagCompensations(self) -> dict:
                ...
                method = methods[int(self.root.get('RawProcess_Settings', 'tlag_meth'))]
                ...
                if method == 'automatic_optimization':
                    ...
                    start_date = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_start_date')
                    start_time = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_start_time')
                    if not start_date: start_date = self.root.get('Project', 'pr_start_date')
                    if not start_time: start_time = self.root.get('Project', 'pr_start_time')
                    configure_TimelagAutoOpt_kwargs['start'] = start_date + ' ' + start_time
                    end_date = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_end_date')
                    end_time = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_end_time')
                    if not end_date: end_date = self.root.get('Project', 'pr_end_date')
                    if not end_time: end_time = self.root.get('Project', 'pr_end_time')
                    configure_TimelagAutoOpt_kwargs['end'] = end_date + ' ' + end_time

                    configure_TimelagAutoOpt_kwargs['ch4_min_lag']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_ch4_min_lag')
                    configure_TimelagAutoOpt_kwargs['ch4_max_lag']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_ch4_max_lag')
                    configure_TimelagAutoOpt_kwargs['ch4_min_flux']  = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_ch4_min_flux')
                    configure_TimelagAutoOpt_kwargs['co2_min_lag']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_co2_min_lag')
                    configure_TimelagAutoOpt_kwargs['co2_max_lag']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_co2_max_lag')
                    configure_TimelagAutoOpt_kwargs['co2_min_flux']  = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_co2_min_flux')
                    configure_TimelagAutoOpt_kwargs['gas4_min_lag']  = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_gas4_min_lag')
                    configure_TimelagAutoOpt_kwargs['gas4_max_lag']  = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_gas4_max_lag')
                    configure_TimelagAutoOpt_kwargs['gas4_min_flux'] = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_gas4_min_flux')
                    configure_TimelagAutoOpt_kwargs['h2o_min_lag']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_h2o_min_lag')
                    configure_TimelagAutoOpt_kwargs['h2o_max_lag']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_h2o_max_lag')
                    configure_TimelagAutoOpt_kwargs['le_min_flux']   = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_le_min_flux')
                    configure_TimelagAutoOpt_kwargs['h2o_nclass']    = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_h2o_nclass')
                    configure_TimelagAutoOpt_kwargs['pg_range']      = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_pg_range')

                    manual_mode = int(self.root.get('RawProcess_TimelagOptimization_Settings', 'to_mode'))
                    if not manual_mode:
                        for k in configure_TimelagAutoOpt_kwargs:
                            configure_TimelagAutoOpt_kwargs = None
                            autoopt_file = self.root.get('RawProcess_TimelagOptimization_Settings', 'to_file')
            ...
            def get_CompensationOfDensityFluctuations(self) -> dict:
                ...
                if out['enable']:
                    ...
                    if out['burba_correction']:
                        ...
                        use_multiple = int(self.root.get('RawProcess_Settings', 'bu_multi'))
                        if use_multiple:
                            for k in kwargs:
                                out[k] = tuple(float(self.root.get('RawProcess_Settings', f'm_{k}_{i}')) for i in range(1, 5))
                        else:
                            for k in kwargs:
                                out[k] = tuple(float(self.root.get('RawProcess_Settings', f'l_{k}_{i}')) for i in ['gain', 'offset'])
        ...
        class _Statistical:
            ...
            def set_SpikeFlag(
                ...
                use_m13 = method
                self.root.set('RawProcess_ParameterSettings', 'despike_vm', str(use_m13))

                # accepted spikes and linterp
                self.root.set('RawProcess_ParameterSettings', 'sr_lim_hf', str(accepted))
                if linterp: self.root.set('RawProcess_ParameterSettings', 'filter_sr', '1')
                else: self.root.set('RawProcess_ParameterSettings', 'filter_sr', '0')
                if use_m13: return  # m13 takes no futher parameters

                # outliers
                self.root.set('RawProcess_ParameterSettings', 'sr_num_spk', str(max_consec_outliers))
            ...
            def get_SpikeFlag(self) -> dict:
                ...
                methods = ['VM97', 'M13']
                out_dict['method'] = methods[int(self.root.get('RawProcess_ParameterSettings', 'despike_vm'))]
                out_dict['accepted'] = float(self.root.get('RawProcess_ParameterSettings', 'sr_lim_hf'))
                out_dict['linterp'] = bool(int(self.root.get('RawProcess_ParameterSettings', 'filter_sr')))
            ...
            def set_AmplitudeResolutionFlag(
                ...
                self.root.set('RawProcess_ParameterSettings', 'ar_lim', str(variation_range))
                self.root.set('RawProcess_ParameterSettings', 'ar_bins', str(bins))
            ...
            def get_AmplitudeResolutionFlag(self) -> dict:
                ...
                out['variation_range'] = float(self.root.get('RawProcess_ParameterSettings', 'ar_lim'))
                out['bins'] = int(self.root.get('RawProcess_ParameterSettings', 'ar_bins'))
                out['max_empty_bins'] = float(self.root.get('RawProcess_ParameterSettings', 'ar_hf_lim'))
            ...
            def set_DropoutFlag(
                ...
                self.root.set('RawProcess_ParameterSettings', 'do_extlim_dw', str(extreme_percentile))
                self.root.set('RawProcess_ParameterSettings', 'do_hf1_lim', str(accepted_central_dropouts))
            ...
            def get_DropoutFlag(self):
                ...
                out['extreme_percentile'] = int(self.root.get('RawProcess_ParameterSettings', 'do_extlim_dw'))
                out['accepted_central_dropouts'] = float(self.root.get('RawProcess_ParameterSettings', 'do_hf1_lim'))
                out['accepted_extreme_dropouts'] = float(self.root.get('RawProcess_ParameterSettings', 'do_hf2_lim'))
            ...
            def set_AbsoluteLimitFlag(
                ...
                # limits
                self.root.set('RawProcess_ParameterSettings', 'al_u_max', str(u))
                self.root.set('RawProcess_ParameterSettings', 'al_w_max', str(w))
                for name, v in zip(
                    ['ts', 'co2', 'h2o', 'ch4', 'n2o'],  # eddypro calls gas4 n2o
                    [ts, co2, h2o, ch4, gas4]
                ):
                    vmin, vmax = v
                    self.root.set('RawProcess_ParameterSettings', f'al_{name}_min', str(vmin))
                    self.root.set('RawProcess_ParameterSettings', f'al_{name}_max', str(vmax))

                # filter
                if filter_outliers:
                    self.root.set('RawProcess_ParameterSettings', 'filter_al', '1')
                    return
            ...
            def get_AbsoluteLimitFlag(self):
                ...
                out['u'] = float(self.root.get('RawProcess_ParameterSettings', 'al_u_max'))
                out['w'] = float(self.root.get('RawProcess_ParameterSettings', 'al_w_max'))

                for name, k in zip(
                    ['ts', 'co2', 'h2o', 'ch4', 'n2o'],  # eddypro calls gas4 n2o
                    ['ts', 'co2', 'h2o', 'ch4', 'gas4']
                ):
                    vmin = float(self.root.get('RawProcess_ParameterSettings', f'al_{name}_min'))
                    vmax = float(self.root.get('RawProcess_ParameterSettings', f'al_{name}_max'))
                    out[k] = (vmin, vmax)

                out['filter_outliers'] = bool(int(self.root.get('RawProcess_ParameterSettings', 'filter_al')))
            ...
            def get_SkewnessAndKurtosisFlag(self):
                ...
                for name, k in  zip(
                    ...
                    soft = float(self.root.get('RawProcess_ParameterSettings', f'sk_sf_{name}'))
                    hard = float(self.root.get('RawProcess_ParameterSettings', f'sk_hf_{name}'))
                    out[k] = (soft, hard)
                return out
            ...
            def get_DiscontinuityFlag(self):
                ...
                for name, k in  zip(
                    ...
                    soft = float(self.root.get('RawProcess_ParameterSettings', f'ds_sf_{name}'))
                    hard = float(self.root.get('RawProcess_ParameterSettings', f'ds_hf_{name}'))
                    out[k] = (soft, hard)
                return out
            ...
            def set_TimelagFlag(
                ...
                self.root.set('RawProcess_ParameterSettings', 'tl_sf_lim', str(soft))
                self.root.set('RawProcess_ParameterSettings', 'tl_hf_lim', str(hard))

                self.root.set('RawProcess_ParameterSettings', 'tl_def_co2', str(co2))
                self.root.set('RawProcess_ParameterSettings', 'tl_def_h2o', str(h2o))
            ...
            def get_TimelagFlag(self):
                ...
                soft = self.root.get('RawProcess_ParameterSettings', 'tl_sf_lim')
                hard = self.root.get('RawProcess_ParameterSettings', 'tl_hf_lim')
                out['covariance_difference'] = (soft, hard)

                out['co2'] = self.root.get('RawProcess_ParameterSettings', 'tl_def_co2')
                out['h2o'] = self.root.get('RawProcess_ParameterSettings', 'tl_def_h2o')
                out['ch4'] = self.root.get('RawProcess_ParameterSettings', 'tl_def_ch4')
                out['gas4'] = self.root.get('RawProcess_ParameterSettings', 'tl_def_n2o')
            ...
            def set_AngleOfAttackFlag(
                ...
                self.root.set('RawProcess_ParameterSettings', 'aa_min', str(aoa_min))
            ...
            def get_AngleOfAttackFlag(self):
                ...
                out['aoa_min'] = float(self.root.get('RawProcess_ParameterSettings', 'aa_min'))
                out['aoa_max'] = float(self.root.get('RawProcess_ParameterSettings', 'aa_max'))
                out['accepted_outliers'] = float(self.root.get('RawProcess_ParameterSettings', 'aa_lim'))
            ...
            def get_SteadinessOfHorizontalWindFlag(self):
                ...
                out['max_rel_inst'] = float(self.root.get('RawProcess_ParameterSettings', 'ns_hf_lim'))
            ...
            def get_EstimateRandomUncertainty(self):
                ...
                out['its_definition'] = its_defs[int(self.root.get('Project', 'ru_tlag_meth'))]
                out['maximum_correlation_period'] = float(self.root.get('Project', 'ru_tlag_max'))
